...
# Same as part 1 but with the difference between arrays implemented
def find_symmetry_axe_smudge(pattern, num_smudge):
    num_rows, num_cols = pattern.shape
    ...
    for idx_col in range(1, num_cols//2 + 1):
        ...
        if np.sum(np.abs(reflected_pattern_left - comparison_pattern_left)) == num_smudge:
            return idx_col
        
        elif np.sum(np.abs(reflected_pattern_right - comparison_pattern_right)) == num_smudge:
              return num_cols - idx_col
    ...
    for idx_row in range(1, num_rows//2 + 1):
        ...
        if np.sum(np.abs(reflected_pattern_up - comparison_pattern_up)) == num_smudge:

            return 100 * idx_row
        
        elif np.sum(np.abs(reflected_pattern_down - comparison_pattern_down)) == num_smudge:
              return 100 * (num_rows - idx_row)
